#!/usr/bin/env python3
"""pdm_coverage_derive.py — the trace/rare-mineral coverage metric (goal + per-product vehicle mg).

Generates dashboard/assets/data/pdm-coverage-data.json — read by state/coverage.ts to score the
33 trace_pdm rare-earth minerals as ONE GROUP against Wallach's plant-derived colloidal-mineral
...
def build_data() -> dict:
    ...
    return {
        "_purpose": (
            ...
            "(judgment) + the sealed Wallach dose claim (goal). Read by state/coverage.ts: the 33 "
