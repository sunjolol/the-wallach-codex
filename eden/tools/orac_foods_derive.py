#!/usr/bin/env python3
"""orac_foods_derive.py -- the ORAC tab's food league-tables NUMBERS.
...
THE RULE (Charter R1/R3 + §00.A): a Wallach number lives in exactly ONE home. No ORAC score
is hand-typed into a view, into view-copy, or into the curation file -- that would be a second,
ungated home, and a value can be sealed as canon, stay green forever, and still be wrong if no
gate can prove it RIGHT. This generator reads every score
...
def _book_cite(claims: dict, books_meta: dict, cid: str) -> str:
    ...
    The food_source claims locate by SCREENSHOT (no printed page), so the cite renders
    book-level rather than as a hand-typed page range. Never hand-typed."""
...
def build_data() -> dict:
    ...
    return {
        "_purpose": "The ORAC knowledge tab's food league-table numbers (sections "
                    ...
                    "curation (R1/R3/§00.A). The curation (orac-foods-curation.json) decides only "
