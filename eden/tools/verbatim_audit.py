...
Luneth rule (SESSION 31, 2026-07-01): a Wallach quote shown under a condition MUST
name that condition (or a registered synonym) in the SHOWN verbatim text, else the
link is unverifiable (indistinguishable from a hallucination). See memory
...
conditions index's `claims_by_role`, so search-only / tier-2 claims (excluded from
the operational indices) are never counted.

Matching (name-or-synonym, Luneth-ruled): the verbatim, OCR-folded and
...
condition-synonyms.json. The synonym map lets faithful paraphrase links
...
def cmd_baseline():
    ...
    out = {"_doc": "Allowlist of KNOWN verbatim-names-condition violations tolerated by the "
                    ...
                    "`verbatim_audit.py baseline`. Shrinks to {} as the remediation campaign "
                    "extends verbatims / drops mappings. NEW violations (not here) FAIL the board.",
           "_count": len(viol)}
