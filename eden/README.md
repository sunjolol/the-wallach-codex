...
4. **Write-protection at the agent layer.** Claude (the agent) is explicitly forbidden from writing to `eden/eden-catalog.json` or `eden/eden-catalog.golden.sha256`. A `check_eden_write_protection` invariant scans saga + memory-change-log for any agent modification to these files and fires CRITICAL on detection.
