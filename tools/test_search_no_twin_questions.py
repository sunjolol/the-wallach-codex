#!/usr/bin/env python3
"""Negative test for search_no_twin_questions (Charter R3 / R7).

Proof artifact (section 00.B "codify, don't promise"). A gate that has only ever been GREEN proves
nothing (memory: negative-control-or-it-proves-nothing). This drives _search_no_twin_questions_impl
directly with planted in-memory enrichment and asserts it REDDENS on the exact defect it was written
for -- the one that reached Luneth's screen on 2026-08-07, three days into a duplicate campaign, on a
89/89 green board:
...
tolerates EVERY twin). 'stale_exception' shows a carve-out that stops firing is itself RED (R9).
...
CASES = [
    ...
    ("stale_exception",
     ...
     "a carve-out that no longer fires is a lie left in the source (R9) -- resolving a blessed twin "
...
def main():
    ...
    if not pinned:
        fails.append(("allowlist_membership",
                      "the in-gate keep-both allowlist is no longer empty; each entry needs Luneth's "
    ...
    # Burn-down meter on the REAL corpus (campaign in progress -- see the module docstring). This is
    # INFORMATIONAL until the merges/rewords land; then ENFORCE_GREEN flips to hard-GREEN before merge.
    print()
