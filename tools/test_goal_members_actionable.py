#!/usr/bin/env python3
"""test_goal_members_actionable.py — NEGATIVE test for the goal-membership gate (R7/R9).
...
A gate that has never been SEEN to fail is theater. This plants each defect the gate claims to
    ...
    PYTHONUTF8=1 python tools/test_goal_members_actionable.py
...
telling the user to do something they cannot do, or quietly stops highlighting something:
   1  trace_pdm member   -> ★ THE DEMO'S OWN BUG. The signed-off demo lists STRONTIUM (a
                            plant-derived element with no individual Wallach amount) under
                            stronger-bones, contradicting the rule stated in its own comment.
                            A ring there is a to-do the user cannot do: nothing they buy can
                            close it individually — the group verdict owns it.
