#!/usr/bin/env python3
"""test_pdm_group_goals_wallach_sourced.py — NEGATIVE test for the group-goal gate (R7).
...
A gate that has never been SEEN to fail is theater. This plants each defect the gate claims to
    ...
    PYTHONUTF8=1 python tools/test_pdm_group_goals_wallach_sourced.py
...
a health claim Wallach never made, or quietly drops one he did:
   ...
   5  neighbouring bleed-> "Colloidal tin" under a DIFFERENT condition's entry must not license
                           ...
                           produced 9 of 12 false positives under the 76-agent adversarial read.
   ...
   7  search-only       -> the Ask-Wallach corpus must never feed an operational surface
                           (search_only_indices_excluded's boundary). A search-only claim may
                           not license a goal dot.
