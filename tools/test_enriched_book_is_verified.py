#!/usr/bin/env python3
"""Negative test for enriched_book_is_verified (§00.B "codify, don't promise" / R7 · R9).
...
flips silent, the exact failure that started this campaign can recur unnoticed.

The SPARING cases matter as much (R9): a verified book, an individually-verified claim, and the
frozen grandfathered backlog must all stay silent, or the gate would redden 2,260 existing claims
and be turned off within a day.

Run:  PYTHONUTF8=1 python tools/test_enriched_book_is_verified.py"""
...
def fcase(name, claims, enriched, ledger, want_clean):
    with tempfile.TemporaryDirectory() as td:
        ...
        (r/"chronicle"/"frontface-ocr").mkdir(parents=True)
        by_book = {}
        ...
        (r/"chronicle"/"frontface-ocr"/"verified.json").write_text(
...
# SPARING: the frozen backlog must not redden 2,260 existing claims.
fcase("grandfathered_claim_spared", CLAIMS,
      ["WAL-CLM-RAW-000001"], LEDGER(gf={"rawbook": ["WAL-CLM-RAW-000001"]}), True)
# R9: the backlog is a waiver, so it must carry a reason a reviewer can check.
