#!/usr/bin/env python3
"""test_mirrors_resolve.py — NEGATIVE test for the mirrors_resolve gate (R7/R9).
...
A gate that has never been SEEN to fail is theater. This plants each defect the gate claims to
    ...
    PYTHONUTF8=1 python tools/test_mirrors_resolve.py
...
WHY EACH CASE EXISTS (every one is a real failure mode, not a shape check):
  ...
  2  slug resolves to nothing -> a typo leaves the tile permanently blank, looking exactly like
                              one of the 53 unmined essentials. Silent by construction.
...
run("8 the REAL sealed data must be GREEN", None, None, want_red=False) if False else None
