...
A mechanism split card may DISPLAY a trimmed literal quote (quote_trim) -- Wallach's words cut before
a trailing sentence -- while the cite composes from the sealed claim (Luneth 2026-07-30: "still
LITERAL Wallach quotes, we're just trimming where the quote stops"). This proves the gate bites when
...
bite without touching the real files. Run:
    ...
    PYTHONUTF8=1 python tools/test_mech_quote_trim_faithful.py
