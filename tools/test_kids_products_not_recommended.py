#!/usr/bin/env python3
"""test_kids_products_not_recommended.py — NEGATIVE test for the kids-exclusion gate (R7/R9).
...
A gate that has never been SEEN to fail is theater. This plants each defect the gate claims to
    ...
    PYTHONUTF8=1 python tools/test_kids_products_not_recommended.py
...
the app just quietly starts recommending kids formulas to adults again):
   ...
   8  essentialSlugs FILTERS-> ★ THE OVER-FIX. Someone "makes it consistent" and filters the
                              ...
                              Luneth explicitly wants them discoverable in. Both directions are
                              defects; a gate that only checked one half would bless this.
   9  brace-swallow         -> ★ THE R9 CASE (same lesson as regimen_state_mutation_routing).
  ...
  10  anti-vacuity          -> no excluded id is a live candidate, so the filter filters nothing
                              ...
                              (negative-control-or-it-proves-nothing).
  11  the REAL files        -> must be GREEN, or the gate is useless.
"""
