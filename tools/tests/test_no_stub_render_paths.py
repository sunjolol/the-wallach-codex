#!/usr/bin/env python3
"""Negative test for no_stub_render_paths (the render-path enforcement floor).

Proof artifact -- sever the old path, then gate it: the gate must GREEN on the real
...
_no_stub_render_paths_impl with synthetic (relpath, text) inputs. Run:
    ...
    PYTHONUTF8=1 python tools/tests/test_no_stub_render_paths.py
...
# one poison per scaffold token, each in the exact shape a prototype page writes it
TOKENS = {
