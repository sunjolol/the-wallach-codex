#!/usr/bin/env python3
r"""Negative test for slot_invariants (slot-system structural guards).

Proof artifact (§00.B "codify, don't promise"). slot_invariants is the STATIC half of the slot
guards: it proves the enforcing CODE EXISTS (SlotDocSchema's .min(1)/.max(4)/.max(20)/.max(7)/
superRefine; writeSlotDoc's setValidated boundary; addSlot's cap-refusal; deleteSlot's
last-refusal + activeSlot promotion). It CANNOT prove the code RUNS correctly -- that is
tools/probes/render_probe_slots.js on the real file:// app. A green static check is not evidence of
runtime correctness, so the split is explicit and the runtime half is a labelled probe, not a
claim.

Each RED case plants one missing guard. If any goes GREEN, that guard is no longer enforced.

Run:  PYTHONUTF8=1 python tools/tests/test_slot_invariants.py
