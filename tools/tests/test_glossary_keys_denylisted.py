#!/usr/bin/env python3
"""Negative test for glossary_keys_denylisted (§00.B "codify, don't promise").
...
24, every one the ordinary-English sense. 129 wrong tooltips, 0 right ones, on a fully green
board. A human found it by reading the page.
...
The SPARING cases matter just as much (tighten, never over-fire). The ban is on the EXACT
...
Run:  PYTHONUTF8=1 python tools/tests/test_glossary_keys_denylisted.py
...
k_cases = [
    # LOAD-BEARING: the exact shape that shipped -- the alias a reader reported.
...
# Reviewability: a denylist entry with no recorded measurement is itself a defect.
