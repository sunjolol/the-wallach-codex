#!/usr/bin/env python3
"""Negative test for no_duplicate_claims -- the same-book duplicate gate.

Proof artifact (§00.B "codify, don't promise"). A gate that has only ever been GREEN proves
nothing. This drives _no_duplicate_claims_impl directly with planted in-memory claims and asserts
it REDDENS on the exact defect it was written for -- the one that reached a reader's screen before
the gate existed:
    ...
    char_offset with the same extracted_at, same subject, same facet -- and a fully green
    board while both rendered.

CASE 'vitaminA_twin_cards' replants that pair with its REAL sealed bytes, recovered from the
commit that removed them. It is the whole point of this file: if it ever goes GREEN, the gate
has stopped biting the class it exists for.
...
claims:
  ...
  * 'cross_book_repeat'       -- Wallach reuses passages between books and each book gets its
                                 own claim, so the signature is scoped to ONE book.

'same_subject_two_facets' is a BOUNDARY case, not a law of nature: it is spared because the
ruled signature requires the same facet. If that ruling ever changes, this case is the thing
that must change with it -- deliberately and on the record, not by a quiet edit to the gate.

The allowlist cases pin the exception mechanism. The ruled keep-both pairs live IN-GATE, not
in .claude/invariant-baseline.json, because that file is INVARIANT-scoped
...
Run:  PYTHONUTF8=1 python tools/tests/test_no_duplicate_claims.py
...
CASES = [
    ...
    ("unenriched_pair",
     ...
     {}, {}, False,
     "two claims with NO enrichment bucket together under (None, None) -- fail closed. A handful "
     "of unenriched claims exist and none of them pair; a future one must not slip because it is "
     "unenriched"),
    ...
    ("same_subject_two_facets",
     ...
     "again as the mechanism lead-in. Spared because the ruled signature requires the same facet; if "
    ...
    # ---- THE ALLOWLIST. The exception mechanism, pinned. ----
    ("approved_pair_spared",
     ...
     "a pair on the in-gate allowlist is spared -- the ruled keep-both pairs must not "
     "red-board a corpus that was already reviewed"),
    ...
    ("stale_exception",
     ...
     "a carve-out that no longer fires is a lie left in the source -- deleting, re-mapping "
...
def main():
    ...
    # The expected count is DERIVED from the literal above. Hardcoding it hid a real
    # defect: two pairs were written twice, so 78 literals collapsed to 76 in the set
    # and the printed number agreed with a figure nobody had rechecked.
    print("%s allowlist_membership   expect=%d ruled pairs        got=%d pair(s)"
          % ("ok  " if pinned else "FAIL", len(expected), len(SHIPPED_ALLOWLIST)))
    if not pinned:
        fails.append(("allowlist_membership",
                      "the in-gate keep-both allowlist changed; each entry is a claim about the "
                      "world and needs a human ruling + this pin updated in the same patch",
