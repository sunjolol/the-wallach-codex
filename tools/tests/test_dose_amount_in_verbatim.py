...
PROVEN on real data before the gate existed:

    a 10x sodium fabrication (3,300 -> 33,000 mg) planted in the sealed shard passed the
    ENTIRE board green -- amounts_wallach_only reported every numeric coverage target as
    tracing to a Wallach dose claim -- while the claim's own verbatim still literally read
    "3,300 mg".
...
weaken the gate without re-running these:
  * CROSS_ROW    -- a verbatim span carries the next nutrient's row (measured at design time,
                    37 of 86 spans named >1 nutrient), so presence-checking accepted a
                    neighbour's number.
...
returns a set of invariant NAMES -- so one entry tolerates EVERY dose claim the gate checks).
Handled in-gate instead, so this gate ships with ZERO exceptions.

Run:  PYTHONUTF8=1 python tools/tests/test_dose_amount_in_verbatim.py
...
CASES = [
    ...
    ("zero_unitless", claim("T-P", ["phosphorus"], 0, "mg", VB_PHOS, BASE), True,
     ...
     "needs ZERO baseline exceptions -- an invariant-scoped exception would tolerate EVERY "
     "dose claim"),
    ...
    ("UNIT_SWAP_safety", claim("T-AG", ["silver"], 400, "mg", VB_SILVER), False,
     "silver 400 mcg -> mg. The project's own safety incident: silver is 400 MCG and "
    ...
    # --- the UNDER-FILLED ROW refinement ---------------------------------------------
    # Fig. 8-1's VITAMIN A row is the only one whose RDA cell is BLANK. Positional column
    # indexing therefore read column 1 as 20,000 and RED-flagged a true value. The printed page
    # settles it: with the RDA cell blank, 5,000 IU IS the true-supplement-need.
