#!/usr/bin/env python3
"""Negative test for frontface_verbatims_clean (§00.B "codify, don't promise").

THE INCIDENT: a human reading the page found raw OCR in USER-FACING quotes -- WAL-CLM-RARE-000336 showing
...
The SPARING cases matter as much (tighten, never over-fire): a mid-line compound hyphen is
CORRECT and must stay silent, and an exception must cover ONLY the claim+detector it names.

Run:  PYTHONUTF8=1 python tools/tests/test_frontface_verbatims_clean.py"""
...
# THE SPACE-BEFORE-NEWLINE wrap. Hell's Kitchen is the ONLY book that wraps this
...
# covers only the unambiguous shapes (tighten, never loosen; the residue stays a labelled WISH).
...
# An exception must cover ONLY what it names -- a wrong detector must NOT excuse it.
...
# An exception for a DIFFERENT claim must not excuse this one.
