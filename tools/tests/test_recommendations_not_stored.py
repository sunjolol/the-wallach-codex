#!/usr/bin/env python3
"""test_recommendations_not_stored.py — NEGATIVE test for the derived-not-stored gate.
...
then asserts GREEN on the real tree. Run:
    ...
    PYTHONUTF8=1 python tools/tests/test_recommendations_not_stored.py
...
coming back". That is the whole reason the rule is structural rather than a convention:
   1  rec LS key         -> ★ THE DEFECT. Someone caches the list "for performance" and the
                            goal→add→remove-goal→remove-item loop becomes possible again —
                            the thing deriving-at-read-time made impossible.
   ...
   5  OVER-FIRE PIN: lcRecentScans_v1 -> ★ The first cut matched any "rec" substring and
                            fired on RECENT SCANS — a real, legitimate recoverable buffer, so
                            that a new scan never silently destroys the last.
                            A gate that reddens a working feature gets deleted, not fixed.
                            This case pins the tightening: RecentScans must stay GREEN.
   6  OVER-FIRE PIN: the word in a COMMENT -> ★ The gate fired on this very module's own
