#!/usr/bin/env python3
"""Negative test for views_no_ciphered_data (a gate is a WISH until proven to fire).

THE RULE: the decorative .ds-cipher glyph-scrambler may wrap view-LOCAL chrome, but never a
value sourced from the state/ or core/ layers -- i.e. never real data. The app's data flows
pillars -> generated artifacts -> core/ -> state/ -> views/, so that import boundary IS the
data/chrome line.

HISTORY: the first cut banned ALL interpolation inside a cipher span. It over-fired on 4
...
Run: PYTHONUTF8=1 python tools/tests/test_views_no_ciphered_data.py
"""
import sys
...
# ── MUST FIRE ────────────────────────────────────────────────────────────────
# The exact regression this gate exists for: the canon-derived count of Wallach's 90 essentials
# handed to the scrambler, which animated it through a run of WRONG numbers on screen before
# settling. A decorative effect must never be fed a real value.
POISON_REAL_COUNT = '''
...
# ── MUST NOT FIRE (real sites in the app; pinned by the boundary refinement) ──
