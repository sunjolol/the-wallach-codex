#!/usr/bin/env python3
"""Negative test for glossary_wellformed (§00.B "codify, don't promise").
...
THE REFINEMENT, added after manual review: a number IS allowed
...
Run:  PYTHONUTF8=1 python tools/tests/test_glossary_wellformed.py
...
print("test_glossary_wellformed -- tightened digit check")
print("-" * 100)
...
cases = [
    ...
    # A vitamin DESIGNATION is part of the name, not a health number. The gate
...
# ---------------------------------------------------------------------------
# The ANCHORED number_exempt block. A digit is allowed ONLY when the
...
GOOD_REASON = "Wallach's own figure, manually verified and ratified in review; tooltip frozen."
...
# ---------------------------------------------------------------------------
# The term+alias KEY-SPACE collision guard. The runtime matcher (state/glossary.ts) folds
# term + every alias into ONE lowercased Map, so a repeated normalized key is a SILENT
# last-write-wins override, not a harmless dup. The OLD check only saw term-vs-term and missed
# all four real collisions (an alias == another entry's term made a dedicated 'myelosclerosis'
# definition dead; meq/l, Supralife, glacial milk were aliases == their own term). These cases
# pin that the full key-space is guarded.
# ---------------------------------------------------------------------------
