// tools/render_probe_regimen_dedup.js — REG-03: a re-added product bumps its dose,
// it does NOT append a second row (which coverage.accumulate would sum into a phantom
// double-count on the 90-field).
//
// Usage: node tools/render_probe_regimen_dedup.js   (exit 0 = PASS, non-zero = FAIL)
//
// WHY THIS EXISTS: views/regimen.ts (typeahead/rec add) and views/scanner.ts (adopt)
// both did `saveRgManual([...loadRgManual(), item])` — a fresh Date.now() id every time,
// so adding the same product twice made TWO rows. views/coverage.ts::addVaultProduct
// already deduped-and-bumped; REG-03 routes all three through the shared state helper
