#!/usr/bin/env python3
"""Negative test for explore_entity_lede_authored (R7 codify-don't-promise / R9).

THE DEFECT, 2026-08-19: the knowledge-topic hero had ALWAYS derived its header from a claim's
...
The SPARING cases matter as much (R9): an essential (type nutrient), a condition, an entity with no
claim, and the frozen grandfathered backlog must all stay silent, or the gate would redden 141
pages and be turned off within a day.

Run:  PYTHONUTF8=1 python tools/test_explore_entity_lede_authored.py"""
...
def fcase(name, ents, claims, topics, grand, want_clean):
    with tempfile.TemporaryDirectory() as td:
        ...
        (r / "chronicle").mkdir(parents=True)
        ...
        (r / "chronicle" / "lede-backlog.json").write_text(
