// recycle popup, replace step — restoring a bin save while all four slots are full opens the
// "Replace a save" step; a pick updates the summary, back/Esc return to the bin list, and
// Replace & restore performs the swap (chosen → bin, saved one → live). UI over the existing
// state op restoreDeletedSlot(deletedAt, replaceSlotId).
