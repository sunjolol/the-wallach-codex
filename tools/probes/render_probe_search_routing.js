// tools/probes/render_probe_search_routing.js — Search resolver INTENT-routing regression guard (exit 0 = PASS).
//
// Usage: node tools/probes/render_probe_search_routing.js
//
// A FOCUSED companion to tools/probes/render_probe_search.js (the comprehensive Ask-Wallach catch-all probe).
...
//
// WHAT IT GUARDS: "how to get more antioxidants" once won the weight-loss warning on the bare token
// "more", ambushing a neutral how-to question with a caution. The fix was an explicit "get more
// antioxidants" intent topic on the top-foods claim (WAL-CLM-HELLS-000014), and these checks pin it.
//
// CAVEAT: these expectations are coupled to the mined corpus. A re-mine can turn a check red with no
// code regression — re-verify the routing against the live resolver before "fixing" it, and never
// merely loosen a check to match new output: a check relaxed to fit its own result stops being a gate.
