// tools/probes/render_probe_knowledge.js — the whole Knowledge drawer, end to end.
//
// Usage: node tools/probes/render_probe_knowledge.js   (exit 0 = PASS, non-zero = FAIL)
//
// Verifies: the K rail item and bare-K toggle the overlay and Esc closes it; the 6-tab bar;
// the Home shelves (conditions, essentials, Explore preview, live-suggest); the Absorption
// landing incl. the pH ladder, fortress scene and the derived enzymes strip; the Products
// vault (list-all, detail panel, composition search); the Essentials route and entity pages
// incl. the shared rare-earth group meter; Conditions pages, the glossary and the umbrella
// tip; drawer search with live highlight; and the Explore tab, topic pages and back-nav.
// Requires puppeteer.
...
(async () => {
  ...
  // Bare-key drawer shortcuts (S/K) are intentionally dead while a blocking overlay owns the
  // screen -- otherwise a shortcut would toggle a drawer behind the modal and leave it open once
  // the modal closes. Dismiss the arrival veil so the probe runs the realistic post-onboarding
  // state (and the bare-K reopen at step 6 is live).
  ...
  // 2b. Tab bar — the six tabs (Home / Absorption / ORAC / Conditions / Explore /
  //     Products), Home active by default. Essentials is deliberately NOT a tab: it
  //     duplicated the Coverage workspace. Corpus and Doctrine are not tabs either.
  ...
  // 2e. Home live-suggest covers Explore TOPICS, not just essentials/conditions. Typing a topic
  //     name ("testosterone") must surface a data-kd-topic result -- an index that omits topics
  //     matches NOTHING and looks identical to "no such topic".
  ...
  // 2f. Foods & Absorption tab -- the curated "second prong" landing: a hero with the
  ...
  //     absorb what you can't break down") is a DESIGNED sequence:
  ...
  const foods = await page.evaluate(() => {
    ...
    return {
      ...
      // section 04: pH ladder + fortress cutaways + inversion callout + one
  ...
  // Essentials is a live route with no tab of its own (it duplicated the Coverage workspace).
  // Enter through Home's "open the full table" link so this probe exercises a real user path
  ...
  // Click Magnesium (one of the most heavily mined essentials) — its tile expands the
  // data-driven entity page.
  ...
  //     the full record grouped by kind, each claim Wallach-cited).
  ...
  const condDeep = await page.evaluate(() => {
    ...
    // lead-in never disagrees with the nutrient block.
  ...
  //     explains the unit (the glossary layer glosses archaic clinical units on sight).
  ...
  const checks = [
    ...
    // Essentials left the tab bar because it duplicated the Coverage workspace, while staying a
    // live route. Asserting its ABSENCE is the point: if it ever reappears here, that is a regression.
    ...
    // The covered/not-covered LEGEND and the per-tile status DOT were removed from THIS drawer
    // screen: they duplicated the Coverage page, which keeps its own. Asserting their ABSENCE
    // makes a future re-addition a regression -- same guard style as "Essentials absent" above.
