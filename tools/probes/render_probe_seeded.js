// tools/probes/render_probe_seeded.js — seeded-regimen coverage classifier check.
//
// Usage: node tools/probes/render_probe_seeded.js
//
// Companion to tools/probes/render_probe.js (which checks the EMPTY-regimen render).
...
//   Aluminum / Yttrium  -> covered  (trace_pdm: the 34 minerals with no individual Wallach
//                                     dose share ONE verdict from the plant-derived-mineral
//                                     aggregate — Σ vehicle mg / the 924 mg goal; here 1200 mg
...
const SEED = { items: [
  ...
  },
  {
    ...
    // sources => 1200 mg >= 0.95 * 924 => the whole 34-mineral group goes covered.
...
(async () => {
  ...
  // The seeded items ARE the whole stack: the HBSP base foundation was removed, so there is
  // no longer a base layer to hide (this used to write rgRemoved_v1 = [-1,-2,-3] to suppress
  // the pre-applied seed).
  ...
  const info = await page.evaluate(() => {
    ...
    return {
      ...
      // The COVERED count comes from the LEDGER, not from a single headline ratio.
      // WHY: "covered" is reached through several incommensurable regimes (wallach 37 ·
      // trace_pdm 34 · dietary_with_clinical_lever 13 · dietary 3 · wallach_collective 2 …),
      // so one "N / 90" fraction would print a count of several different kinds of thing as
      // a single number. The DISTRIBUTION replaces the ratio; the assertion below fires on
      // the same threshold.
