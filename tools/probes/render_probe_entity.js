// tools/probes/render_probe_entity.js — the unified ENTITY PAGE (essential variant).
//
// Usage: node tools/probes/render_probe_entity.js   (exit 0 = PASS, non-zero = FAIL)
//
// Drives the essential entity page end-to-end via the Knowledge drawer:
// open -> Essentials -> click Calcium -> assert hero · store-authored
...
(async () => {
  ...
  // Prove the enrichment-question filter path: pick a record card whose
  ...
  // entity page carrying the --prod modifier (renderProductDeep). Assert on .kd-ep--prod WITH a
  // product name: an empty panel would mean the product id did not resolve, and asserting on the
  // container alone would pass on that failure.
