// tools/probes/render_probe_adopt.js — adopt scanned product → regimen → coverage.
//
// Usage: node tools/probes/render_probe_adopt.js   (exit 0 = PASS, non-zero = FAIL)
//
// Drives the core value path: score a label with the native engine, then fire the
// SAME write cascade the Scan·Confirm·Result "Add to regimen" button fires —
...
//   - covered tiles on the Coverage surface increase (saveRgManual →
//     regimen:changed → coverage recompute cascade).
// The shipped view drives adopt from a real upload→OCR→confirm→result flow a
...
// Starts from an empty regimen so the coverage delta is clean. (The rgRemoved_v1 write below
// is vestigial: state/regimen.ts reads that key once at migration and then ignores it.)
// Requires puppeteer.
...
(async () => {
  ...
  const adopt = await page.evaluate((label) => {
    ...
    if (bridge) {
      // Mirror views/scanner.ts adopt: a user_scanned item through the regimen write chokepoint.
