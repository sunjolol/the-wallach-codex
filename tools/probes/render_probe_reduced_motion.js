// tools/probes/render_probe_reduced_motion.js — the flash-hazard gate.
//
// Usage: node tools/probes/render_probe_reduced_motion.js
//
// WHY THIS EXISTS. design-system.css capped animation-DURATION under
...
// elements were measured looping at ~100Hz (ds-stat-pulse, ds-travel-top/-bottom and
// ds-pulse-animate among them; two of the seven have since been deleted outright): a strobe
// served to precisely the users who had asked for LESS motion, well past the 3-per-second
// flash threshold. No gate could see it — the CSS declared the right intent and did the
// opposite, so reading it proved nothing.
//
// The anchor is the RENDERED computed style in a real browser under the real media feature
// (the truth-anchoring rule: pin to something that cannot itself drift, not to a source-text
// grep that a comment or a reworded rule could satisfy). It generalises: any future animation
// added ANYWHERE that loops fast under a reduce preference reddens this probe, not just the
// original block.
//
...
// (the fastest real ambient loop is 1.15s — the scanner progress shimmer), so the band is
// unambiguous.
const FLASH_S = 0.05;
