// tools/probes/render_probe_omega.js — the omega / essential-fatty-acid group meter.
//
// Usage: node tools/probes/render_probe_omega.js
//
// WHY THIS EXISTS. There was once ZERO probe coverage for any omega: render_probe_entity
// hardcodes Calcium, render_probe_knowledge uses Magnesium/Dysprosium, and no probe named
// an omega at all. That blind spot let a
...
// The dose multiplier MUST ride on label.servings. state/coverage.ts::readScale tries
...
async function run(browser, seed) {
  ...
  await page.evaluateOnNewDocument((s) => {
    // Clear first — this probe reuses ONE browser across 4 worlds, and the slot migration
    // PERSISTS rgSlots_v1 on first read, so a prior world's slot would otherwise survive and
