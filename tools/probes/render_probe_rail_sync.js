// tools/probes/render_probe_rail_sync.js — the rail highlight tells the truth, + the nameplate face.
//
// Usage: node tools/probes/render_probe_rail_sync.js   (exit 0 = PASS, non-zero = FAIL)
//
// WHY THIS EXISTS. The rail's active class is DERIVED state, and the shell used to re-derive
// it only on the paths IT drove (rail click, Esc, the search->knowledge swap). Closing a
// drawer from INSIDE — its own [X], or the search scrim — flipped the drawer's isOpen without
// telling the shell, so the button stayed lit for a drawer that was already gone.
//
// The fix is an emitted 'drawer:toggled' (core/events.ts). A static check would only prove the
// emit EXISTS, never that it RUNS; this drives each drawer through its OWN close affordance and
// asserts the highlight actually drops. Against the pre-fix bundle every case must FAIL — that
// negative control is what makes a pass mean anything.
