// tools/probes/render_probe_mech_shape.js — the element-header SHAPE guard.
//
// Usage: node tools/probes/render_probe_mech_shape.js   (exit 0 = PASS, non-zero = FAIL)
//
// WHY THIS EXISTS. The mechanism renderer grew a second shape: alongside the legacy
...
// WAS the fixed chassis that forced every header into the same shape — the reason for the refactor.
// But it meant touching the one function that draws every already-approved header — the set
// captured in tools/gate-fixtures/mechanism-sections.json (currently selenium, copper, vitamin-a, zinc).
//
...
// tools/gate-fixtures/mechanism-sections.json, which was captured from the PRE-refactor bundle.
//
...
//
// Scope, honestly: this proves the captured headers still render byte-for-byte what they rendered
// before, and that the frame (tan content box · disclaimer · Best-Youngevity-sources dock) still
// wraps them. It is not a full test of the composed path — calcium, magnesium and vitamin-a declare
// `blocks` in mechanism-clarity-data.json and each is covered by its own per-element probe.
// The composed path's DATA contract is gated by the
...
const GOLDENS = path.join(REPO, 'tools', 'gate-fixtures', 'mechanism-sections.json');
...
(async () => {
  ...
  for (const slug of SLUGS) {
    ...
    // The fixed frame (tan content box, disclaimer, sources dock) must still wrap the body.
