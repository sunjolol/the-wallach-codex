...
1. **Wallach corpus primary** — a Wallach-authored BOOK (with year), or a protocol within one. **BOOKS ONLY — no lectures/transcripts (Luneth 2026-07-05: "we are only using things Wallach has directly said in his books").** The **7** in-housed books — **ids exactly as registered in `eden/corpus/books-meta.json`, which is the authority; this list is a convenience mirror**: `dddl-3e-2011` (Dead Doctors Don't Lie, 2011) · `rare-earths` (Rare Earths: Forbidden Cures, 1994) · `lets-play-doctor` (Let's Play Doctor, 1995) · `epigenetics` (Epigenetics, 2014) · `immortality` (Immortality, 2008) · `iaiyh` (It's All In Your Head, 2020) · `hells-kitchen` (Hell's Kitchen, 2015). **Corrected 2026-07-15:** this read 'The 6 in-housed books' and omitted `hells-kitchen` — a book that is registered AND carries 8 sealed claims, i.e. the §00.A allowlist did not cover a source already in use. The old shorthand tokens (`rbs`, `eps`) never matched the registry ids either. **The ONLY valid source for a recommended amount, dose, range, target, deficiency indicator, or health claim.** (The old `wallach-lecture` token is RETIRED — it was never used; all sealed claims are book-sourced.)
...
- **LIVE (Charter R2 — `amounts_wallach_only`, critical):** every NUMERIC coverage target in `dashboard/assets/data/essentials-targets-data.json` carries a `source_claim_id` resolving to a sealed Wallach `dose` claim that maps the essential's slug; a Youngevity-sourced or unsourced amount is RED. **The gate recomputes the full transform CHAIN (2026-07-09):** it anchors each target's `provenance.original_{low,high,unit}` to the sealed claim's dose, independently re-runs the documented chain (upper-of-range → IU-factor → ×1.54 / round-2sf), pins every IU factor to the known physical constant, and byte-compares the result to the posted number — so a wrong/planted factor, a hand-typed value, or a stale artifact reddens, not merely a missing `source_claim_id`. Targets DERIVE from Wallach base-line supplement-program dose claims (`eden/tools/targets_derive.py`) — Youngevity Healthy Body Start Pak label sums are gone. Essentials with no Wallach maintenance dose show no number (an honest gap, blueprint §7.1), never a fabricated one.
- **LANDED (Phase D-c, 2026-07-05):** `citations_reference_registry` (book refs = `book_id`, no hand-typed cite), `no_hand_duplicated_canonical` (no canonical essential name re-stored outside `essentials-canon`), and `prose_contained` (no prose in a fact field) — all LIVE over the clean Charter surface (corpus claims + canon + catalog + the corpus-derived targets/coverage-layout artifacts).
- **WISH remaining (Phase E/F/G):** the FULL `prose_contained` (verbatim → a claim pointer, summary → the per-essential prose store — only meaningful once clean post-mining stances exist) + extending all three gates to the legacy not-yet-collapsed embeds + the legacy view scaffold. Labeled WISH, not sold as safe (R7).
