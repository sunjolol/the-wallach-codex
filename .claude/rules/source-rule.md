...
## Enforcement
_Transitional (Phase A): the live gates below guard the rotten middle layer this overhaul is collapsing; they retire with it. The durable Charter gate lands in Phase C._
- **CURRENT (live — guards the rotten layer, retires with it):** `wallach_stance_source_rule` (critical) — every `wallach_stance.citation` in `knowledge/essentials-targets.json` cites an allowlisted primary; `wallach_stance_embed_sync` — the stance dict is byte-equal in the dashboard embed. The allowlist lives in-code in `tools/invariants.py` (`check_wallach_stance_source_rule`); if it grows, update it there and this note in the same patch.
- **TARGET (Charter R2 — `amounts_wallach_only`, a WISH until Phase C):** every recommended amount carries a `source_claim_id` resolving to a Wallach claim; a Youngevity-sourced amount is RED. Paired with `citations_reference_registry` (book refs = `book_id`, display composed, never hand-typed). These arrive in Phase C with the derivation pipeline; until then they are labeled WISH, not sold as safe (R7).
