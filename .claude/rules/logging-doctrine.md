...
4. **Teach without bloating the files.** Narrative lives in Git history, the chronicle, and the Creator's Log — NOT in app code (code carries only truthful WHY/decision comments per §00.B — session narrative is never inline; the old "JSDoc-only" letter was pruned SESSION-12). The history carries the story; the code stays clean. Size budgets still bind; completeness is balanced against them.
...
7. **Plain-language first, then the complete technicals.** Every substantive log entry (Creator's Log `detail`, build-log line, commit body) OPENS with a short plain-language line — what happened and why, in terms a non-expert can follow (Luneth is learning to code; write it for him) — THEN the full technical specifics beneath. Readability is added ON TOP of the complete record, never by cutting it: the technicals stay whole and uncapped. Keep the human layer tight — a little narrative, not a wall. Separate the plain lead from the technicals with a **blank line** so the two layers visually split (the Profile panel renders it via `white-space: pre-wrap`; git and the build-log show it as a paragraph break). **Honesty outranks format (the ranking that makes this safe):** never invent, inflate, or pad to satisfy the shape. A short TRUE entry beats a padded one; if there is genuinely little to say in plain terms, the honest short version wins, and that is NOT a violation. (Minimum-force: doctrine-only as of SESSION-43 — deliberately NO machine floor, because a minimum-length check would breed the exact padding this rule forbids; promote to a machine check only if the discipline proves insufficient.)
...
- **Writing a commit:** address the future learner. OPEN with the plain-language what + why (rule 7), then state the technical what, the why, the verification + its output, and any deferred follow-up. The honesty rule binds — if a step was skipped or could not run, say so.
...
- The sacred covenant's core machine guard is LIVE: the git-anchored **append-only invariant** `creators_log_append_only` (committed entries must remain a line-prefix of the file — catches any delete/truncate/edit), alongside `creators_log_well_formed` / `_digest_synced` / `_embed_synced` / **`_bundle_synced`**. The last was ADDED 2026-07-02 after a silent-staleness incident: three round-close entries fired correctly into the ledger AND the source embed (so `_embed_synced` stayed green) but were never rebuilt into `dist/main.js`, so the in-app Profile log silently omitted them — the file:// app inlines the embed at BUILD time, not runtime. `creators_log_bundle_synced` (critical) pins to the newest ledger id inside the BUILT bundle (§00.B #11: anchor to the artifact the user loads, not a source-vs-source pair), and `stop_round_close.py` hard-blocks a close whose bundle is stale. This was NOT a firing failure — the log fired — so the earlier-DECLINED round-close **firing check** and log-content **hard cap** stay declined (SESSION-43 minimum-force holds: the log fires reliably and `detail` is uncapped). Rule 7 (plain-language-first + honesty-outranks-format) is likewise doctrine-only for now; promote to a machine check only if the discipline proves insufficient.
- This doctrine is the WHY. If a part can be made machine-checkable WITHOUT inviting gaming (rule 7's floor problem), promote it per §00.B.
