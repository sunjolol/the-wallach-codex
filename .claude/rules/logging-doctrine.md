...
- The sacred covenant's core machine guard is LIVE: the git-anchored **append-only invariant** `creators_log_append_only` (committed entries must remain a line-prefix of the file — catches any delete/truncate/edit), alongside `creators_log_well_formed` / `_digest_synced` / `_embed_synced`. A **round-close firing check** and a log-content **hard cap** were CONSIDERED and DECLINED SESSION-43 (minimum-force: the log already fires reliably with no observed failure, and `detail` is already uncapped so nothing is being truncated — adding either would risk a working system for no real gain). Rule 7 (plain-language-first + honesty-outranks-format) is likewise doctrine-only for now; promote to a machine check only if the discipline proves insufficient.
