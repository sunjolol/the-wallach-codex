...
Each mined claim routes to its correct home: **tier-1 operational** (conditions / symptoms / essentials) OR **search-only** (broader Wallach guidance), per the per-book policies in the corpus rules. The two are not exclusive — a dual-home claim is both searchable and tier-1.
...
## Enforcement
- **LIVE:** `mining_coverage_accounted` (the completeness denominator), `mined_pages_clean`, the full R5 mine-gate family, `substance_triage_accounted`. A silent DROP is machine-caught at book-completion.
