...
- Pillar seals — the per-pillar hash gates: `corpus_integrity` · `products_hash_integrity` · `catalog_integrity` · `graphics_integrity`. Write protection is a **hook, not an invariant**: `tools/hooks/pre_write_guard.py` blocks any write to a file carrying a `*.golden.sha256` sibling.
  - **What a seal proves, exactly:** the pillar has not CHANGED since sealing. It never proves the pillar is RIGHT. A value that was wrong when sealed stays wrong, sealed, forever — and the gate stays green. (2026-07-15: the canon's mineral tiers were bootstrapped from a UI mockup, sealed, and read as canon for three weeks under a green board. Tamper-evidence is not correctness.)
  - **Corrected 2026-07-15:** this line cited `eden_hash_integrity` + `eden_write_protection` as live invariants. NEITHER EXISTS. `eden_hash_integrity` was DELETED 2026-07-08 (commit `761495be`) together with the transitional `eden-catalog.json` it guarded — see the tombstone at `tools/invariants.py:357`. `eden_write_protection` has no definition anywhere in the repo. Both were cited here, and in `charter.md`, and were repeated to the user as working gates.
