...
| `recommendations_not_stored` | no derived list in LS or any artifact | **LIVE** (2026-07-16, the live Coverage build) — no rec-shaped LS key, no rec-list artifact, and `state/recommender.ts` is PURE (no localStorage, no `core/storage` import), so the rule cannot rest on the ranker choosing not to persist. Negative test: `tools/test_recommendations_not_stored.py` (7 cases, incl. 2 R9 over-fire pins: `lcRecentScans_v1` and the word in a comment) |
| `goal_members_actionable` | a goal may only name an essential you can ACT on individually; no per-goal `total` | **LIVE** (2026-07-16, NEW — not in the original table). Blocks a goal naming a `trace_pdm` (no individual Wallach amount — one shared verdict) or a fiat-covered slug (H/C/N/O — nothing to take), proves every member resolves on the board, and RED-flags a per-goal `total` (a denominator; the denominator is always 90). ★ Its load-bearing check is the FIAT-DRIFT seam: `coverage_layout_derive.FIAT_COVERED_SLUGS` must equal `state/coverage.ts::FOUNDATIONAL_PRESENT_SLUGS` — the list is written twice because Python cannot import TS. Negative test: `tools/test_goal_members_actionable.py` (9 cases) |
| `dose_default_sourced` | every default = a claim id, a `directions` field, or an explicit rung-3 flag | **WISH (labelled, R7).** NOT shipped with the Coverage build: the rail reads the dose, it does not ASSIGN the default (readScale resolves override → `label.servings` → 1). The gate belongs with the dose-ladder work that gives rung 3 its explicit flag. Until then the rung-3 default rests on D7 + review, and the rail renders it as *your setting*, never as a recommendation |
...
| `render_probe_coverage_add_remove` | `+` → field relights → remove → recommendation returns | **LIVE** (2026-07-16) — 22 checks: the veil appears once and gates on name + ≥1 goal · `+` adds through §31 and the field relights · **the dose stepper MOVES THE COUNTS** (Luneth's named requirement; the demo's is inert) · stepping down restores the EXACT counts · remove dims the field and the rec RETURNS (derived, never stored) · the denominator never moves at any step · no kids product ever appears. **Negative control run:** with the stepper made inert (the demo's behaviour) the probe REDs on exactly the 3 dose checks — it catches the bug it exists for |
