...
> **★★ PARTIALLY SUPERSEDED — read `chronicle/next-chunk.md` + memory [[demo-vision-not-letter]] + [[search-is-ask-wallach-popup]] FIRST.** The PHILOSOPHY below (foundation-first · no copying the demo's shortcuts · contained prose · Luneth visual sign-off per surface) is CURRENT and correct. But specific **§4 / H2 BUILD notes are RETIRED and must NOT be acted on**: "reuse `renderIntakeMeter`/`renderEssentialSources`/`renderWhyThisNumber`", "port the claim primitives", the old tab set, "cross-surface to Search", "900px". Live truth: drawer **950px** · tabs **Home·Essentials·Conditions·Explore·Products** (label-only pills; Corpus/Doctrine dropped) · topic pages live IN the Knowledge drawer · canonical demo = ONLY `temporary/knowledge-drawer-prototype.html`. Where this doc conflicts with `next-chunk.md`, next-chunk wins.
