...
- **epigenetics is screenshot-spread** (like immortality, NOT PDF-clean). Screenshots in `temporary/Epigenetics…/`, Tesseract OCR cache `tools/frontface/ocr-cache/ocr-epig-*.json`, `corr_shots.py` ShotBook (BOOKS map already has "epigenetics"). **Reuse the committed screenshot template** `chronicle/ratify-unverified-2026-08-19/_templates/` (scripts `01..08` + README — the exact working immortality scripts): `03_shotbook_locate_screenshots.py` maps each verbatim → its `Screenshot (N).png`; `04_vision_fleet_workflow_SCREENSHOT.js` is the vision fleet (reads the dual-monitor render, book on LEFT ~48%); `06/07/08` do corrected-verbatims / bundle / .txt-patch. purity-status "raw".
