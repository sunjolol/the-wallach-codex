...
# ★★★★ 2026-07-21 (SESSION 4) — ★ SEALED + GLOSSARY R9 SHIPPED (kv 369) · NEXT = THE PDM CLARITY-BLOCK RECALL
**Board 77/77. Committed + pushed.** The 3 plant-derived claims are SEALED and the glossary numbers are now GATED, not smuggled. The actual point of this whole enrichment — recalling that content onto the 34 PDM element pages — is the NEXT chunk and NEEDS his visual sign-off.

## What shipped this chunk (committed)
1. **corpus_seal kv 368→369** — RARE-000069/070/071 (the colloidal-minerals "Glacial Milk" trio) sealed with fuller claim_text + verbatim + offsets (646672/647037/648877). 1335 claims, verify PASS. Every out-of-verbatim number verified against Wallach's own Chapter-10 words FIRST (98%/8-12% → RARE-000061; 5-12% = his "from five to 12 per cent" @652107; 60-72/3-20 in 071; terrace-conversion @652169). Pre-seal all-7-books draft-vs-shard diff CLEAN (only these 3 claims, only claim_text/verbatim/locator).
2. **4 glossary hovers rewritten** (colloidal minerals / Glacial Milk / rock flour / parent rock) — sized PER-TERM after he flagged them first lazy, then a wall-of-text. Lesson: don't swing to the opposite extreme; find the right size. [[directives-are-guidelines-stay-balanced]]
3. **★ THE GATE STORY (the real lesson).** The numbers he wanted (98%, 60-72) tripped `glossary_wellformed` (critical) — which bans health numbers in tooltips because the glossary is the ONE content layer OUTSIDE the §00.A source gates, so a bare number there is an un-anchored, un-auditable Wallach claim. He rejected stripping them and asked for a real exception, not the lazy path. **R9 fix: the gate now ALLOWS a number IFF the entry carries an anchored `number_exempt` — a reason + claim_ids that RESOLVE and whose digit-runs CONTAIN the number.** "No numbers ever" → "no UNANCHORED numbers"; non-gameable (can't cite an unrelated claim; the number must literally appear in the cited claim). colloidal→RARE-000061, parent rock→RARE-000071. Files: `invariants.py` (`_corpus_claim_digit_runs` + `_glossary_number_exemption_valid`), `core/schemas/glossary.ts` (optional `number_exempt`), `glossary.json` (_doc + 2 blocks), `test_glossary_wellformed.py` (15→22 cases; 5 non-gameable negatives). [[gate-exceptions-are-world-claims]] [[the-instrument-lies-before-the-eye]]
4. **Goal-chip 1px label nudge** (coverage.ts `gchip__label` span + workspace-coverage.css) — prior screenshot-approved; bundled here because the derived artifacts (main.js/search-index) entangle it with the seal.

## ★ NEXT CHUNK — THE PDM CLARITY-BLOCK RECALL (the enrichment's ACTUAL point) — NEEDS HIS VISUAL SIGN-OFF
His plan, restated so it's not re-litigated: plant-derived content lives in ONE source and is RECALLED on each of the 34 plant-derived mineral element pages — NO 34 duplicate entries. This IS the R4 clarity-pass model: `essential-special-cases.md` entries **6 + 9**; prototype `dashboard/assets/data/fatty-acid-clarity-data.json`, which already renders in the entity deep-dive.
- The 34 PDM element pages are SPARSE: 22 of 60 minerals have ≤2 claims; the rare-earths (Cerium, Erbium, Lutetium, Praseodymium, Samarium, Scandium, Ytterbium, Zirconium + 9 more) have exactly **1 claim each**. Recalling the colloidal concept + the plant-processing/98% education onto each gives even a bare Cerium page the "how plant-derived minerals work" story from one authored source.
- **The RICH numbered education (98% vs 8-12%, the mechanism) belongs HERE** — on the page, attributed, §00.A-clean. That's where the numbers Luneth wanted live at FULL strength; the tooltip is only the anchored teaser.
- Build model: a plant-derived clarity store (shaped like fatty-acid-clarity-data.json) + entity-page render + a render probe + **HIS visual sign-off** (visual-verification gate — do NOT ship silently). Data-DRIVE it off flags (like the omega work); NEVER branch the entity view on a slug literal (`entity_render_is_projection` will reject it).

## Still open (carried, not blocking)
- **RARE-000074/000088 MISFRAMED + RARE-000089 EDITORIAL_TAIL** among the plant-derived ACCEPT drafts — do NOT land without his ruling (same trap as EPIGEN-000089).
- The ratification queue / worth-a-look pass / 2nd-independent-pass to 99% (budget-gated; he is careful on spend).
- Regimen + Scanner rebuilds (`views/regimen.ts` + `views/scanner.ts` still burn).
[[small-batch-build-test-log-mandate]] [[wallach-efa-omega9-stance]] [[gate-exceptions-are-world-claims]] [[workflow-token-budget-guard]]
