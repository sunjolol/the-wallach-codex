...
- **★ FOLLOW-UP (the standing rule below):** the Ratification Queue artifact still shows these 23 as TO-RULE. Move all 23 from `pending.json` → `applied.json` and REPUBLISH to the same url, or they re-export on his next paste. After that the "to rule" count is 0.
