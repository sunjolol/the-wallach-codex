...
- **NEXT ORDER (Luneth):** (1) **Journey J1-J4** — both `views/journey.ts` AND `state/journey.ts` are throwing scaffolds (types only; every fn throws). Auto-derive the timeline from REAL activity (existing chokepoints already log real scan/regimen/coverage events); a fresh dashboard shows a clean empty-state; NO fake seed; check-ins PRIVATE (never exported). Visual contract: `dashboard/components/drawer-journey-v3-PROPOSAL.html` (4 tabs: Timeline/Goals/Check-ins/Milestones). Template to mirror: the implemented `views/knowledge.ts` + its 2A wiring in `main.ts`. (2) Command palette (⌘K).
