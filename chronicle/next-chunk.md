...
- **The blind spot (never repeat):** this stance saturates the corpus (**302 lines / 342 occurrences** of the gluten/celiac/leaky-gut/malabsorption/villi vein — the old 283 undercounted) and has **sealed Good/Bad-Foods flyers** (`eden/graphics/{Bad,Good}-Foods-{Front,Back}.jpg`), yet the app NEVER surfaced it across many sessions — Luneth (lifelong Wallach follower) couldn't tell if the app even knew. MASSIVE repeated error. Meta-lesson: proactively surface the IMPORTANCE of prevalent Wallach stances; don't wait to be asked. (Memory [[surface-prevalent-wallach-stances]].)
- **The build ("total overhaul without the overhaul"):** (A) a **persistent absorption caveat + the mantra across the board** — everywhere coverage/% shows — so it can NEVER be missed; (B) a dedicated **Foods & Absorption** section — modern, PERSUASIVE UX that guides the user to eliminate gluten as a lifestyle (partial is not enough). ★ **Luneth 2026-07-12 RULING: re-articulate the good/bad foods FROM THE BOOKS, NO flyer images** — the sealed flyers carry Gluten-Intolerance-Group branding + GiG-sourced lists, so we do NOT surface the JPGs; every food claim is Wallach-book-cited (this SUPERSEDES the blueprint §3/§4 + [[diet-absorption-second-prong]] "surface the flyers" lines).
...
## ★ PROGRESS (2026-07-12, session in flight) — 2 commits pushed
**Decisions LOCKED with Luneth:** (1) Foods home = a dedicated Foods & Absorption **tab** in Knowledge **+** backing Explore topic pages. (2) **Flyers: re-articulate from the BOOKS, NO images** (see ruling above). (3) Charged content (homosexuality/intersex) kept OUT of all diet content; scope verbatims to avoid it; STOP + ask if inseparable. (4) Mine scope = exhaustive on DISTINCT stances, DEDUP near-identical repeats.
**Shipped:**
- **Pilot** (commit 80a6e635): 2 Explore topic pages — **Gluten** + **Nutrient Absorption** — built by ENRICHING already-sealed claims (NO seal). Confirmed **DUAL-HOME works** (`search_index_derive.validate()` has no must-be-search-only check) → enrich a tier-1 claim to make it searchable AND keep it operational. Voice+structure signed off; **Luneth is the LIVE gate** — real feedback pends a live look.
- **Crown-jewel seal** (commit 3f908026, knowledge_version **325**): 3 NEW Epigenetics search-only claims sealed — EPIGEN-000140 mantra ("you are what you absorb"), 000141 Mayo 115M gluten stat, 000142 two-prong fix — now filling the Gluten + Absorption hero ledes.
- **Good/bad-foods mine** (commit pending this close, knowledge_version **326→327**): mined + sealed **39** good/bad-food claims re-articulated FROM THE BOOKS (8-theme workflow → reviewed 45 → deduped 39). 10 NEW food Explore pages live: Cooking Oils & Fried Foods, Phytates & Oxalates, Sugar & Caffeine, Soft Drinks, Eggs, Salt, Meat, Dairy, Processed Meats, Cruciferous/Goitrogens, Water. Gate-fixes en route: 1 bad facet, 2 bare "ALS"→gloss (reseal 327), goiterogenic glossary alias. Deferred: 3 rare-earths quotes (narrow-column OCR hyphenation → recapture in rare-earths purification) + a few raw-book verbatim OCR blemishes (B,,/pd/sugarin).
**Approach that works:** most of "pepper the stance across the corpus" = ENRICH the ~219 existing on-theme claims (NO seal) into the diet entities; only genuinely-NEW content (mantra/stat/good-bad-foods) needs finalize→seal. Entities so far (13): `gluten`, `absorption`, `phytates` + 10 food topics (`salt`, `sugar`, `carbonated_beverages`, `dietary_oils`, `eggs`, `meat`, `dairy`, `processed_meat`, `cruciferous_vegetables`, `water`) in search-entities.json (UNSEALED → plain safe_write, no reseal). ~180 existing on-theme claims still to bulk-enrich (no seal). The IAIYH dual-prong quote already lives inside sealed **IAIYH-000004** (enrich, no mine).
**NEXT (in order):** (a) **Phase 2 — build the Foods & Absorption TAB** (the live persuasive surface Luneth judges live), reading the now-rich diet/absorption search claims; a `graphics_derive` is NOT needed (no flyer images); (b) bulk-enrich the ~180 remaining on-theme claims (no seal) into the entities to deepen the pages; the OLD (a)/(b) below fold into these. (LEGACY note — do not re-run: the good/bad-foods mine is DONE.) Build the Foods & Absorption **tab** (the live persuasive surface Luneth judges live) per the page-replication technique below + a new `graphics_derive` is NOT needed (no images); (c) **Part A** — the persistent absorption caveat across coverage surfaces (RESTRAINT: [[persuade-dont-shove-restraint]] — ONE great surface + rare load-bearing pointers, NEVER saturate); (d) the **Coverage-tab OVERHAUL** (now in scope, Luneth 2026-07-12 — rebuild to Knowledge-tab quality, absorb the diet framing natively, fix the 2 fake coverage numbers: goal-card proportional fake + regimen slot hardcoded literals).
