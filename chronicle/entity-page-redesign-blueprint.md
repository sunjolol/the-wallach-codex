...
- **Converged:** keep the current app layout; **Knowledge stays a DRAWER, widened 420 → 950 px**, adopting the browse-shell inner design; **ONE consolidated search** ends the two-search weirdness.

**Locked decisions:**
1. **No layout change.** Rail + drawers stay. Knowledge = a slide-in drawer at **950 px** (NOT full-screen, NOT master-detail).
...
- **P1 — the 950 px Knowledge drawer — ✓ COMPLETE + SIGNED OFF (2026-07-10).** All 5 tabs built on the real shell + visually signed off: Home · Essentials · Conditions · Explore · Products (demo: `temporary/knowledge-drawer-prototype.html` @950px). Sticky in-room filter on Conditions + Products (pinned flush under the header via a `.sh-stick` shield with negative `top` covering the body's 16px padding band — Chromium pins sticky to the CONTENT-box top so margin can't lift it, only a negative `top`; symmetric ~12px gap above/below). First section-label per tab trimmed 15px via `.ep-seclabel--tight` (hero + between-section gaps untouched). Products = a flat list of 215 sorted by nutrient coverage, rows `name · covers N/90 · price`, green accent, detail deferred to P3. **Tiles/rows WIRED ✓ (2026-07-10, 3 verified chunks, gitignored prototype):** clicking Calcium → the D1 page and Osteoporosis → the D2 page, both opening in-drawer (mounted in a nested `#drawer-search-mount.ep-standalone` host so the approved D1/D2 styling applies verbatim). Branch pills resolve name→slug and open a built page if one exists, else a labeled "lands in the real build" stub (Option A — only the 2 exemplars have hand-built page data; generalized per-entity rendering is Phase H). A **breadcrumb trail + history stack** replaced the single back button — a parent crumb returns to that exact page (Luneth: "back should go back"). **Remaining:** Luneth verifies the breadcrumb FEEL (morning); the "PROTOTYPE v4" footer was removed per his note.
