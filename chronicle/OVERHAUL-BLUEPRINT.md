...
- `corpus_embed.py`: `indices/` + `claims/` → `corpus-embed.json`, imported into the bundle by `state/corpus.ts` (esbuild JSON import). ✓ guarded by `corpus_embed_synced`.
...
**App shell + styles**
- `dashboard.html` — **CHANGE** (pure shell; embeds removed, D1). `styles/design-system.css{,.golden}` — **KEEP** (sealed). Other `.css` — **KEEP**. `legacy-dashboard.css` — **DELETE in Phase E** (still `<link>`'d and styles the live `#legacy-workspace-host` fallback; sever when that host retires).
- `legacy-dashboard.js` — **DELETE in Phase E** (its logic is fully ported to TS and it is NOT loaded/imported — but the `#legacy-workspace-host` fallback it sits beside is still mounted by `main.ts:65`; sever when that host retires). `creators-log-handler.js` — **REVIEW/CONSOLIDATE**.
...
- **Phase E — Per-surface finalize (§5).** Update views to the new derived data (coverage/goals); Knowledge already clean; scanner wall + `scanner_user_items_marked`. **Retire the `#legacy-workspace-host` fallback once every rail item has its own mount, then DELETE `legacy-dashboard.js`+`.css` and their now-dead invariants (`legacy_css_contained` + the concat scans).** Per-surface probes + visual sign-off.
