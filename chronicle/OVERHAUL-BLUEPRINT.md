...
**EXTENDED + LOCKED 2026-07-09:** Section 7's remaining phases (G book-mining, H, I) were expanded into the mining → Search-validation → seal → app-completion → design roadmap (mining method as-needed per element/condition; page-by-page / vein-sweep retired 2026-07-17); the mining HOW is `.claude/rules/mining-veins.md`.
...
- **Phase G — Book mining (as-needed) → Search validation → seal.** _Expanded + LOCKED 2026-07-09; mining doctrine REWRITTEN 2026-07-17 (page-by-page / vein-sweep retired by Luneth): `.claude/rules/mining-veins.md`._ Finish the corpus by mining AS-NEEDED, per element / per condition — never a full-page sweep, never a silent guess, with a mandatory manual review pass from Luneth (the books are peppered with errors). Honesty is gated by `mining_coverage_accounted` (a book flips to `complete` only when every page/section is claim-bearing OR reviewed-empty+reason). Sub-phases:
  - **G-1 ✓** Epigenetics dose table · **G-2 ✓** glossary (2026-07-09).
  - **G-3** — adopt the as-needed mining doctrine; seed `eden/tools/mining-coverage.json` page/section coverage-maps for the remaining books.
  - **G-4** — finish **Immortality**: resume the element encyclopedia at `La-Lanthanum` (char 299,378) through Zr (canon elements tier-1; the ~21 non-canon inert/radioactive → search-only or reviewed-empty), then scan Ch 4–12 narrative for other mineable content.
  - **G-5** — full **DDDL** re-mine (as a new book, as-needed per element/condition; source already pristine).
  - **G-6** — the **3 newly-bought books**: Luneth reads + photographs the chapters he wants; Claude in-houses them (adding each `book_id` to the sealed `books-meta` allowlist with year) and mines the photographed content.
