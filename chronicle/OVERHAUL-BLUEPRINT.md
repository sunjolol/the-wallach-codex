...
**App shell + styles**
- `dashboard.html` — **CHANGE** (pure shell; embeds removed, D1). `styles/design-system.css{,.golden}` — **KEEP** (sealed). Other `.css` — **KEEP**. `legacy-dashboard.css` — **DELETE**.
- `legacy-dashboard.js` — **DELETE** (the parked 4th copy). `creators-log-handler.js` — **REVIEW/CONSOLIDATE**.
...
- **Phase A — Governance reset + noise purge (S8 runs HERE, first).** Fix the operating files so all later work runs under correct rules: rewrite `CLAUDE.md` + `REVIEW.md` to the Charter/pillar model; audit the 12 `.claude/rules/`; categorize the 43 invariants (delete the 3 rotten-layer guards + `legacy_css_contained`); rewrite `next-chunk.md`, supersede `genesis/02`; audit the 74 memory files. DELETE the clear dead weight: `wallach-refresh/`, `legacy-dashboard.js`+`.css`, `corpus-changelog.md`, `genesis/01`, `dashboard/.git/`, `__pycache__`. Add `no_operating_doc_contradiction`. *Nothing new is built — just a clean, correct foundation.*
...
- **Phase E — Per-surface finalize (§5).** Update views to the new derived data (coverage/goals); Knowledge already clean; scanner wall + `scanner_user_items_marked`. Per-surface probes + visual sign-off.
